<template>
  <b-container>
    <b-alert :show="error['seminar'] ? true :false" variant="info">
      Please select seminar date from calender
    </b-alert>
    <b-row>
       <b-col md="8" sm="12" lg="8" class="calender-root">
        <b-row>
          <b-col md=4 class="label01">
            <label v-text="`${currentSeminar.title || ''}`"></label>
          </b-col>
          <b-col md=8 class="label02" style="text-align:right;width:0px;">
            <label
        ...
        <b-col class="mt-3">
            <b-row>
              <b-col sm="12">
                <label :for="`type`" class="labelsi col-form-label"><b>Seats Available:</b></label>
              </b-col>
              <b-col sm="12">
            ...
            <b-row>    <!--v-if="currentSeminar.isFull"-->
              ...
              <b-col sm="12">
                <span class="form-control labelse">{{currentSeminar.FormatedCost}}</span>
              </b-col>
            </b-row>
            </div>
        ...
        <b-row v-if="!currentSeminar.isFull" class="mt-2">
          ...
          >
            <b-card no-body class="mb-1">
              <b-card-header header-tag="header" class="p-1" role="tab" style="background-color:#E8ECEF;">
                <div class="card_header">
                  <div>
                    <span><b>{{`Registrant-${index +1}`}}</b></span>
                  </div>
                  <div>
                    <!-- <b-button
                      @click="addNewReg"
              ...
              <b-collapse :id="'collapse-'+index" visible accordion="my-accordion" role="tabpanel">
                <b-card-body>
                  ...
                  >
                    ...
                    <!-- <b-form-input
                      type="number"
                      :state="l_error[`registrants.${index}.phoneNumber`] ? false : true"
                      v-model="item.phoneNumber"
                      required
                      placeholder="Enter phone number (Numbers only without dash)"
                    ></b-form-input> -->
                     <the-mask placeholder="Enter Phone number" :class="{'is-invalid':l_error[`registrants.${index}.phoneNumber`] ? true:false,'is-valid':l_error[`registrants.${index}.phoneNumber`] ? false:true}" label="US Phone" mask="+1 (###) ###-####" class="form-control" v-model="item.phoneNumber" ></the-mask>
                  </b-form-group>
                  <b-form-group
                    label-cols-sm="4"
                    label-cols-lg="3"
...
this.loadData(this.$route.params.id);
  ...
  methods: {
    ...
    addNewReg() {
      let _this = this;
      let length = this.registrants.length+1 || 0;
      if (length > this.currentSeminar.empty_seat) {
        this.$swal({allowOutsideClick: false,
          ...
          // text: `The selected seminar has only ${
          //   this.currentSeminar.empty_seat
          // } available slots. Would you like to proceed?`,
          type: "warning",
    ...
    registrantsError() {
      ...
      this.registrants.forEach((r_item, r_index) => {
        ...
        if (!r_item.phoneNumber) {
          _this.error = {
            ..._this.error,
            [`registrants.${r_index}.phoneNumber`]: [
              "Phone mumber is required"
            ]
          };
        } else if (
          ...
          _this.error = {
            ..._this.error,
            [`registrants.${r_index}.phoneNumber`]: [
              "The phone Number is not in valid format"
            ]
          };
        } else {
